...
def get_answer_from_engine(bottype, query, btntype, situ_plus):
    # 챗봇 엔진 서버 연결
...
def query(bot_type):
    ...
    try:
        if bot_type == 'TEST':
            ...
            ret = get_answer_from_engine(bottype=bot_type, query=body['query'], btntype=body['btntype'], situ_plus=body['situ_plus'])
            return jsonify(ret)
